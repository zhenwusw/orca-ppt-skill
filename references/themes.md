...
**加一套配色必须跑 `npm run palettes`。** 它核三对组合的对比度 —— 正文/页面底、
面板上的文字/面板、次要文字/页面底。低饱和配色最容易骗过眼睛：莫兰迪的豆沙面板配白字
单看没问题，量出来只有 2.76:1，离屏幕两米就读不动。这个 bug 就是它抓到的。

不核强调色的对比度：`.t-accent` 的大字是**有意的低对比装饰**（Editorial Forest 封面
那个粉色「finer.」），不是缺陷。
