...
同一套版式换一组配色（马卡龙、深色版、单色版）就是**另一个主题文件**，不需要别的机制：
`editorial-forest-macaron.css` 自然就是选择器里的一张新卡。Keynote 也是这么做的 ——
「基本（白色）」和「基本（黑色）」是并列的两张卡，不折叠。

**变体名用 `·` 分隔，不要用括号**：`主题：Editorial Forest · 马卡龙（配色取自 …）`。
括号留给出处说明，读主题名时会在左括号处截断 —— 写成「Editorial Forest（马卡龙）」
的话，界面上显示的就是「Editorial Forest」，和原版分不出来。

变体和原版**各自挑自己的标签**：配色变了，适合的场合也可能变（马卡龙不一定还适合
「对外路演」）。别默认照抄。
